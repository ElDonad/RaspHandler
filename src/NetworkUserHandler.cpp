...
nlohmann::json NetworkUserHandler::processItem(nlohmann::json toProcess, nlohmann::json client)
{
    ...
    {
        std::cout<<"Demande de r�cup�ration d'aiguillage recue"<<std::endl;
    ...
    {
        std::cout<<"erreur de r�ception de paquet"<<std::endl;
    }
...
std::vector<std::string> NetworkUserHandler::socketReceive(ASocket::Socket* toReceive, bool wait)
{
    ...
    int hasToTransfer;// retour direct de la socket, utile dans ce scope car utilis� dans la boucle conditionnelle
    ...
    if (!message.empty())
    {
        ...
        {
            if (*it == '\u0008')//TODO : faire àa mieux (soit un tag spécifique de fin de message, soit un header)
            {
